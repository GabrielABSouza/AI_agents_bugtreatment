...
def processar_bugs():
    try:
        ...
        # Criar a equipe com configuracoes
        crew = Crew(
                        ...
                        try:
                            ...
                            try:
                                with psycopg2.connect(**db_params) as conn_retry, conn_retry.cursor() as cur_retry:
                                    ...
                                    cur_retry.execute(
                                        "INSERT INTO classificacao_prioridade (chamado_id, prioridade) VALUES (%s, 'Menor')",
                                        (bug_id,)
                                    )
                                    conn_retry.commit()
                                    logger.info("Classificacao de severidade salva com valor padrão 'Menor'")
                    ...
                    if desenvolvedor:
                        ...
                        try:
                            ...
                            with psycopg2.connect(**db_params) as conn_status, conn_status.cursor() as cur_status:
                                ...
                                if status_normalizado == 'Em desenvolvimento':
                                    cur_status.execute(
                                        "INSERT INTO andamento_chamados (chamado_id, responsavel, status) VALUES (%s, %s, 'Em desenvolvimento')",
                                        (bug_id, responsavel_limitado)
                                    )
                                elif status_normalizado == 'Em teste':
                                    cur_status.execute(
                                        "INSERT INTO andamento_chamados (chamado_id, responsavel, status) VALUES (%s, %s, 'Em teste')",
                                        (bug_id, responsavel_limitado)
                                    )
                                elif status_normalizado == 'Aguardando review':
                                    cur_status.execute(
                                        "INSERT INTO andamento_chamados (chamado_id, responsavel, status) VALUES (%s, %s, 'Aguardando review')",
                                        (bug_id, responsavel_limitado)
                                    )
                                elif status_normalizado == 'Resolvido':
                                    cur_status.execute(
                                        "INSERT INTO andamento_chamados (chamado_id, responsavel, status) VALUES (%s, %s, 'Resolvido')",
                                        (bug_id, responsavel_limitado)
                                    )
                                elif status_normalizado == 'Fechado':
                                    cur_status.execute(
                                        "INSERT INTO andamento_chamados (chamado_id, responsavel, status) VALUES (%s, %s, 'Fechado')",
                                        (bug_id, responsavel_limitado)
                                    )
                                else:
                                    # Valor padrão seguro
...
def normalizar_prioridade(prioridade):
    ...
    valores_permitidos = ['Critico', 'Grave', 'Menor']
    ...
    if 'criti' in prioridade_lower or 'críti' in prioridade_lower or 'alta' in prioridade_lower:
        return 'Critico'
    elif 'grav' in prioridade_lower or 'média' in prioridade_lower or 'media' in prioridade_lower:
        return 'Grave'
    else:
        return 'Menor'
...
def normalizar_status(status):
    ...
    # Valores permitidos exatos conforme a restrição CHECK na tabela
    valores_permitidos = ['Atribuido', 'Em desenvolvimento', 'Em teste', 'Aguardando review', 'Resolvido', 'Fechado']
    ...
    if 'atri' in status_lower or 'aberto' in status_lower:
        return 'Atribuido'
    elif 'desen' in status_lower or 'progr' in status_lower:
        return 'Em desenvolvimento'
    elif 'test' in status_lower:
        return 'Em teste'
    elif 'review' in status_lower or 'revis' in status_lower or 'aguard' in status_lower:
        return 'Aguardando review'
    elif 'resolv' in status_lower or 'conclu' in status_lower or 'finaliz' in status_lower:
        return 'Resolvido'
    elif 'fecha' in status_lower or 'encerr' in status_lower:
        return 'Fechado'
    else:
        return 'Atribuido'
